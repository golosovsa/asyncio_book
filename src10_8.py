"""
Сервис backend-for-frontend для товаров
с. 290
"""
import asyncio
...
async def all_products(request: Request) -> Response:
    async with aiohttp.ClientSession() as session:
        ...
        cart = asyncio.create_task(session.get(f'{CART_BASE}/users/2/cart'))
        ...
        if products in pending:
            ...
            return web.json_response({'error': 'Не удалось подключиться к серверу товаров'})
            ...
            return web.json_response({'error': 'Ошибка сервера при подключении к сервису товаров.'})
        else:
...
async def get_products_with_inventory(session: ClientSession, product_response) -> list[dict]:
    ...
    inventory_task_to_product_id = {
        get_inventory(session, product['product_id']): product['product_id']
        for product in product_response
    }

    inventory_done, inventory_pending = await asyncio.wait(inventory_task_to_product_id.keys(), timeout=1.0)

    product_results = []

    for done_task in inventory_done:
        if done_task.exception() is None:
            product_id = inventory_task_to_product_id[done_task]
            ...
            logging.exception(
                ...
                exc_info=inventory_task_to_product_id[done_task].exception()
            )

    for pending_task in inventory_pending:
        pending_task.cancel()
        product_id = inventory_task_to_product_id[pending_task]
        product_results.append(create_product_record(product_id, None))
