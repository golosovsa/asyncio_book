...
async def listen_for_messages(reader: StreamReader, message_storage: MessageStorage):
    while (message := await reader.readline()) != b'':
        await message_storage.append(message.decode())

    await message_storage.append('Сервер закрыл соединение.')
